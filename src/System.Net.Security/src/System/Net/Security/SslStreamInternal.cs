...
namespace System.Net.Security
{
    ...
    internal class SslStreamInternal
    {
        ...
        private static readonly ArrayPool<byte> PinnableBufferCache = ArrayPool<byte>.Shared;
        ...
        private void FreeReadBuffer()
        {
            ...
            {
                PinnableBufferCache.Return(_internalBuffer);
        ...
        private void EnsureInternalBufferSize(int newSize)
        {
            if (_internalBuffer == null || _internalBuffer.Length < newSize)
            {
                ...
                if (newSize <= PinnableReadBufferSize)
                {
                    ...
                    _internalBuffer = PinnableBufferCache.Rent(PinnableReadBufferSize);
                }
                else
                {
                    _internalBufferFromPinnableCache = false;
                ...
                if (wasPinnable)
                {
                    PinnableBufferCache.Return(saved);
                }
            }
        ...
        private void StartWriting(byte[] buffer, int offset, int count, AsyncProtocolRequest asyncRequest)
        {
            ...
            if (count >= 0)
            {
                do
                {
                    ...
                    int encryptedBytes;
                    byte[] outBuffer = PinnableBufferCache.Rent(PinnableWriteBufferSize);
        ...
        {
            PinnableBufferCache.Return(buffer);
        }
